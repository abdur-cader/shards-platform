...
export default function ShardsClient({ shards }: Props) {
    ...
    return (
        <>
            ...
            <section className={`${layout === "masonry" ? "max-w-7xl" : "max-w-6xl"} mx-auto justify-center items-center`}>
                <div
                    className={
                        layout === "masonry"
                ...
                >
                    {shards.map((shard) => (
                        <FadeInOnScroll key={shard.id} className={layout === "masonry" ? "break-inside-avoid mb-6" : ""}>
                            <ShardComponent
                                slug={shard.slug}
                                title={shard.title}
