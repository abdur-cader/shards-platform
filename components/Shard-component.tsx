...
export default function ShardComponent({ slug, title, description, username, lineClamp, image, className }: ShardProps) {
    return (
        <div>
            <Link href={`/shards/${slug}`}>
                <div className={`${className} relative h-full w-full border border-gray-800 rounded-xl cursor-pointer group overflow-hidden bg-[#0a0a0a] hover:rotate-1 hover:scale-105  transition-all duration-300 hover:border-gray-700`}>
                    ...
                    <div className="relative flex flex-col h-full p-4 space-y-4">
                        ...
                        <div className="flex items-center justify-between pt-4 border-t border-gray-800 group-hover:border-gray-700 transition-colors duration-300">
                            ...
                            Created by <span className="truncate max-w-[7rem] block text-orange-700 group-hover:text-orange-500">
                            @{username}
                            </span>
                            </span>
                        </div>
